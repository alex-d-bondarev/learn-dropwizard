...
start_service:
	...
	java -javaagent:'./jacoco/jacocoagent.jar=destfile=./target/jacoco-m/jacoco-m.exec,output=tcpserver' -jar target/com-learn-dropwizard-1.0-SNAPSHOT.jar server config.yml

jacoco_dump:
	@echo "Starting jacoco report dump"
	java -jar ./jacoco/jacococli.jar dump --destfile ./target/jacoco-m/jacoco-m.exec

jacoco_report:
	@echo "Starting jacoco report generation"
	java -jar ./jacoco/jacococli.jar report ./target/jacoco-m/jacoco-m.exec --classfiles ./target/classes/com --html ./target/jacoco-m
...
jacoco_combined_report:
	...
	java -jar ./jacoco/jacococli.jar report ./target/jacoco-c/jacoco-c.exec --classfiles ./target/classes/com --html ./target/jacoco-c
